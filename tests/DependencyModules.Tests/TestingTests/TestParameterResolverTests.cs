...
public class TestParameterResolverTests
{
    ...
    public async Task DataTakesTheLeadingParametersAndTheContainerTakesTheRest()
    {
        var (resolver, provider) = Build(
            nameof(Samples.DataThenService),
            services => services.AddSingleton<IThing, Thing>()
        );
    ...
    {
        var arguments = await Resolve(
            nameof(Samples.RegisteringAttribute),
            services => services.AddSingleton<IThing, Thing>()
        );
    ...
    {
        var arguments = await Resolve(
            nameof(Samples.AbstainingThenAnswering),
            services => services.AddSingleton<IThing, Thing>()
        );
    ...
    public void SetupIsOfferedEveryParameter()
    {
        ...
        new TestParameterResolver(
            ContextFor(nameof(Samples.TwoRegisteringAttributes))
    ...
    private static async Task<object?[]> Resolve(
        string methodName,
    ...
    private static ITestMethodContext ContextFor(string methodName) =>
        new StubContext(
            typeof(Samples).GetMethod(methodName, BindingFlags.Public | BindingFlags.Static)!
        );
    ...
    private static class Samples
    {
        ...
        public static void UnregisteredWithInjectedValue(
            [InjectValues("supplied")] NeedsAValue value
        ) { }

        public static void TwoRegisteringAttributes(
            [RegistersOther] IThing first,
            [RegistersOther] IThing second
        ) { }
    }
