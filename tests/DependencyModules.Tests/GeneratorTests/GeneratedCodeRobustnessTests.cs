...
public class GeneratedCodeRobustnessTests
{
    ...
    public void GeneratedCode_ProducesNoWarnings()
    {
        ...
        var warnings = result
            ...
            .Where(diagnostic =>
                generatedTreePaths.Contains(diagnostic.Location.SourceTree?.FilePath ?? "")
            )
            .ToArray();

        Assert.True(
            warnings.Length == 0,
            "Generated code produced warnings:"
                + Environment.NewLine
                + string.Join(
                    Environment.NewLine,
                    warnings.Select(w => $"  {w.Id} {w.GetMessage()}")
                )
        );
    }
    ...
    public void GeneratedCode_ProducesNoWarnings_ForANullableTypeArgument(
        string _,
        string attribute
    )
    {
