...
public static class AttributeUtility
{
    ...
    public static T? GetTestAttribute<T>(this MethodInfo methodInfo)
        where T : class
    {
        var returnAttribute =
            methodInfo.GetOrderedCustomAttributes().FirstOrDefault(a => a is T)
            ?? methodInfo
            ...
            ?? methodInfo
                .DeclaringType?.GetTypeInfo()
                .Assembly.GetOrderedCustomAttributes()
                .FirstOrDefault(a => a is T);
    ...
    /// <returns>The first matching attribute of the specified type, or null if no attribute is found.</returns>
    public static T? GetTestAttribute<T>(this ParameterInfo parameterInfo)
        where T : class
    {
    ...
    public static IEnumerable<T> GetTestAttributes<T>(this MethodInfo methodInfo)
        where T : class
    {
        ...
        if (methodInfo.DeclaringType != null)
        {
            ...
            returnList.AddRange(
                methodInfo.DeclaringType.GetTypeInfo().GetOrderedCustomAttributes().OfType<T>()
            );
        }
    ...
    public static IEnumerable<T> GetTestAttributes<T>(this ParameterInfo parameterInfo)
        where T : class
    {
        var returnList = new List<T>();
    ...
    {
        return assembly.GetCustomAttributes().Order(new SortCustomAttribute(assembly));
    }

    private static IEnumerable<Attribute> GetOrderedCustomAttributes(this Type type)
    {
        return type.GetCustomAttributes().Order(new SortCustomAttribute(type.Assembly));
    }
    ...
    private static IEnumerable<Attribute> GetOrderedCustomAttributes(
