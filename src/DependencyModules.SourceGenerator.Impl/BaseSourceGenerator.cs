...
public abstract class BaseSourceGenerator : IIncrementalGenerator
{
                ...
                (options, _) =>
                {
                    ...
                    if (
                        options.GlobalOptions.TryGetValue(
                            "build_property.DependencyModules_RegistrationType",
                            out var value
                        )
                    )
                    {
                    ...
                    if (
                        options.GlobalOptions.TryGetValue(
                            "build_property.DependencyModules_LogOutputDirectory",
                            out var logOutputFolderValue
                        )
                    )
                    {
                    ...
                    if (
                        TryGetBoolean(
                            options,
                            "DependencyModules_RegisterGenerator",
                            out var generator
                        )
                    )
                    {
                    ...
                    if (
                        TryGetBoolean(
                            options,
                            "DependencyModules_AutoGenerateModule",
                            out var autoGenerateEntryValue
                        )
                    )
                    {
                    ...
                    if (
                        TryGetBoolean(
                            ...
                            out var generateFactoriesValue
                        )
                    )
                    {
                    ...
                    if (
                        options.GlobalOptions.TryGetValue(
                            "build_property.GeneratedCodeStyle",
                            out var codeStyleValue
                        )
                    )
                    {
    ...
    )
    {
        ...
        if (
            !options.GlobalOptions.TryGetValue("build_property." + propertyName, out var raw)
    ...
    /// </para>
    /// </remarks>
    protected virtual void SetupRootGenerator(
        IncrementalGeneratorInitializationContext context,
    ...
    private bool TriggersOnDefaultModuleAttribute()
    {
        ...
        return moduleAttributes.Length == 1
            && moduleAttributes[0]
                .Equals(KnownTypes.DependencyModules.Attributes.DependencyModuleAttribute);
    }

    private IncrementalValuesProvider<ModuleEntryPointModel> CreateSourceValueProvider(
        IncrementalGeneratorInitializationContext context
    )
    {
        var classSelector = new SyntaxSelector<
        ...
        return context
            .SyntaxProvider.CreateSyntaxProvider(classSelector.Where, GenerateEntryPointModel)
            .WithComparer(new ModuleEntryPointModelComparer());
    }

    protected virtual ModuleEntryPointModel GenerateEntryPointModel(
    ...
    private ModuleEntryPointModel GetClassEntryPointModel(
        GeneratorSyntaxContext context,
        CancellationToken cancellation,
    ...
    )
    {
        ...
        if (typeDeclarationSyntax.BaseList != null)
        {
            foreach (var baseType in typeDeclarationSyntax.BaseList.Types)
            {
                ...
                if (
                    typeDefinition is GenericTypeDefinition
                    {
                        TypeDefinitionEnum: TypeDefinitionEnum.InterfaceDefinition,
                        Name: "IDependencyModuleFeature"
    ...
    private ModuleEntryPointModel GetCompilationUnitSyntaxEntry(
        GeneratorSyntaxContext context,
        CancellationToken cancellation
    )
    {
        ...
        {
            if (
                syntax is GlobalStatementSyntax
                {
            ...
            {
                if (
                    context.SemanticModel.GetSymbolInfo(expressionStatementSyntax.Expression).Symbol
                ...
                )
                {
                    ...
                    var moduleInterface = typeSymbol?.AllInterfaces.Any(x =>
                        x.GetTypeDefinition()
                            .Equals(KnownTypes.DependencyModules.Interfaces.IDependencyModule)
                    );

                    if (
                        moduleInterface.GetValueOrDefault(false)
    ...
    private DependencyFlags GetDependencyFlags(GeneratorSyntaxContext context)
    {
        var onlyRealm = false;
        ...
        if (context.Node is TypeDeclarationSyntax typeDeclarationSyntax)
        {
            var module = typeDeclarationSyntax
                .DescendantNodes()
