    ...
    where T : IClassModel
{
    ...
    protected virtual void CustomImplementation(
    ...
    private static ClassDefinition ConstructClassDefinition(IConstructContainer container, T model)
    {
        ...
        attributeClass.AddBaseType(
            KnownTypes.DependencyModules.Interfaces.IDependencyModuleProvider
        );
    ...
    )
    {
        foreach (var propertyInfoModel in model.PropertyInfoModels)
        {
            ...
            {
                propertyType = TypeDefinition.Get(
                    propertyType.TypeDefinitionEnum,
            ...
            property.DefaultValue = new WrapStatement(
                CodeOutputComponent.Get(stringBuilder.ToString()),
                "default(",
                ")!"
            );
        }
        ...
        if (model.Parameters.Count > 0)
        {
            ...
            foreach (var constructorParameter in model.Parameters)
            {
                var field = attributeClass.AddField(
                    ...
                    constructorParameter.ParameterName + "Field"
                );

                var parameter = constructor.AddParameter(
                    constructorParameter.ParameterType,
                    constructorParameter.ParameterName
                );
