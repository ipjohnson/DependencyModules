...
public static class InterceptedMemberReader
{
    ...
    )
    {
        ...
        foreach (var candidate in EnumerateMembers(serviceType))
        {
            ...
            {
                unsupported =
                    $"'{candidate.Name}' is static, which cannot be forwarded through an instance";
                return false;
            }
    ...
    )
    {
        ...
        declarations.Add(
            new InterceptedDeclarationModel(
    ...
    )
    {
        ...
        {
            unsupported =
                $"'{property.Name}' is a ref struct and cannot be held for the duration of a call";
            return false;
        }
    ...
    )
    {
        ...
        members.Add(
            EventAccessor(@event.RemoveMethod.Name, @event.Name, AccessorForm.EventRemove, type)
        );
    ...
    private static IReadOnlyList<InterceptedParameterModel>? ReadParameters(
        IReadOnlyList<IParameterSymbol> declared,
        string memberName,
        out string? unsupported
    )
    {
