...
public static class AttributeModelHelper
{
    ...
    )
    {
        ...
        {
            var results = GetAttributes(context, attributeLists.Value, cancellationToken, filter);
    ...
    )
    {
        ...
        return operation.Type != null
            ? InternalAttributeModel(context, attribute, operation)
            : null;
    }
    ...
    )
    {
        ...
        if (attribute.ArgumentList != null)
        {
            ...
            {
                var operationValue = context.SemanticModel.GetOperation(
                ...
                {
                    var name = attributeArgumentSyntax
                        .NameEquals.Name.ToString()
                        .Replace("=", "")
                        .Trim();

                    properties.Add(new AttributeArgumentValue(name, constantValue));
                }
                else
                {
                    arguments.Add(new AttributeArgumentValue("", constantValue));
                }
            }
        }
        ...
        return new AttributeModel(type, arguments, properties, GetInterfaces(context, attribute));
    }

    private static object? GetOperationValue(
        SyntaxTransformContext context,
        IOperation operationValue
    )
    {
    ...
    private static bool IsUnboundGeneric(TypeSyntax type) =>
        ...
        && generic.TypeArgumentList.Arguments.All(argument =>
